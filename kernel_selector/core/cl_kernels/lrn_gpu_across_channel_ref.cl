...
KERNEL (lrn_gpu_across_channel_ref)(const __global UNIT_TYPE* input, __global UNIT_TYPE* output)
{
    ...
    const uint batch_id     = b_f / INPUT_FEATURE_NUM;
    const uint feature_id   = b_f % INPUT_FEATURE_NUM;
    ...
    const uint feature_id   = b_f / INPUT_BATCH_NUM;
    const uint batch_id     = b_f % INPUT_BATCH_NUM;
#endif    
