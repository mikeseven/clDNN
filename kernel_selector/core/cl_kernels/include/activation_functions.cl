...
#define UNIT_MIN min
#else
#define UNIT_MAX fmax
#define UNIT_MIN fmin
#endif
...
#define ACTIVATION_RELU(input)                          (UNIT_MAX(UNIT_VAL_ZERO, input))
#define ACTIVATION_RELU_NEGATIVE_SLOPE(input, slope)    isinf(TO_UNIT_TYPE(slope)) ? ((input >= UNIT_VAL_ZERO) ? \
                                                        input : -TO_UNIT_TYPE(slope)) : \
                                                        (UNIT_MAX(input, UNIT_VAL_ZERO) + TO_UNIT_TYPE(slope) * UNIT_MIN(input, UNIT_VAL_ZERO))
#define ACTIVATION_ELU(input, alpha)                    (UNIT_MAX(input, UNIT_VAL_ZERO) +  \
                                                        TO_UNIT_TYPE(alpha) * (exp(UNIT_MIN(input, UNIT_VAL_ZERO)) - UNIT_VAL_ONE));
#define ACTIVATION_CLAMP(input, m, n)                   (UNIT_MAX(TO_UNIT_TYPE(m), UNIT_MIN(TO_UNIT_TYPE(n), input)))
