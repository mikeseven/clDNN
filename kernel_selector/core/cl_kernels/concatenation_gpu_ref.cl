...
KERNEL (concatenation_gpu_ref)(__global UNIT_TYPE* input, __global UNIT_TYPE* output, uint output_offset_in_concat_axis)
{
    ...
    uint input_offset  = INPUT0_OFFSET + d1*INPUT0_PITCHES[DIM_1] + d2*INPUT0_PITCHES[DIM_2] + d3*INPUT0_PITCHES[DIM_3];
    ...
    for (size_t idx = 0; idx < INPUT0_SIZES[DIM_0]; ++idx)
    {
        output[output_offset] = ACTIVATION(input[input_offset], NL_M, NL_N);
        input_offset  += INPUT0_PITCHES[DIM_0];
