...
topology build_test_transpose(const std::string& weights_dir, const engine& engine, layout& input_layout,
                              const bool mean_subtract)
{
    ...
    if (mean_subtract)
    {
        ...
        const auto reorder_mean    = file::create({engine, join_path(weights_dir, "test_transpose.nnd")});
    ...
    topology_inst.add(img_transpose);
