...
void run_topology(const execution_params &ep)
{
    ...
    if (ep.topology_name == "alexnet")
        primitives = build_alexnet(ep.weights_dir, engine, input_layout, gpu_batch_size, false);
    else if (ep.topology_name == "vgg16" || ep.topology_name == "vgg16_face")
        primitives = build_vgg16(ep.weights_dir, engine, input_layout, gpu_batch_size, false);
    else if (ep.topology_name == "googlenet")
        primitives = build_googlenetv1(ep.weights_dir, engine, input_layout, gpu_batch_size, false);
    else if (ep.topology_name == "gender")
        primitives = build_gender(ep.weights_dir, engine, input_layout, gpu_batch_size, false);
        ...
        primitives = build_squeezenet(ep.weights_dir, engine, input_layout, gpu_batch_size, false);
    else
