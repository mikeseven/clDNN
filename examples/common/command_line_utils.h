...
/// @brief Exact std::pair wrapper with support for use as command-line parameter.
template <typename Elem1Ty, typename Elem2Ty>
    struct pair : std::pair<Elem1Ty, Elem2Ty>
{
    /// @brief Base std::pair type.
    using base_type = std::pair<Elem1Ty, Elem2Ty>;
    ...
    // Additional conversions from std::pair:
    // ReSharper disable once CppNonExplicitConvertingConstructor
    template <typename Ty = base_type, typename = std::enable_if_t<std::is_copy_constructible<Ty>::value>>
    pair(const base_type& std_pair) noexcept(std::is_nothrow_copy_constructible<Ty>::value)
        : base_type(std_pair) {}

    // ReSharper disable once CppNonExplicitConvertingConstructor
    template <typename Ty = base_type, typename = std::enable_if_t<std::is_move_constructible<Ty>::value>>
    pair(base_type&& std_pair) noexcept(std::is_nothrow_move_constructible<Ty>::value)
        : base_type(std::move(std_pair))
    {}

    template <typename Ty = base_type>
    auto operator =(const base_type& std_pair) noexcept(std::is_nothrow_copy_assignable<Ty>::value)  // NOLINT(cppcoreguidelines-c-copy-assignment-signature,
                                                                                                     //        misc-unconventional-assign-operator)
        -> std::enable_if_t<std::is_copy_assignable<Ty>::value, pair&>
    {
        base_type::operator=(std_pair);
        return *this;
    }

    template <typename Ty = base_type>
    auto operator =(base_type&& std_pair) noexcept(std::is_nothrow_move_assignable<Ty>::value) // NOLINT(cppcoreguidelines-c-copy-assignment-signature,
                                                                                               //        misc-unconventional-assign-operator)
        -> std::enable_if_t<std::is_move_assignable<Ty>::value, pair&>
    {
        base_type::operator=(std_pair);
        return *this;
    }
...
/// @brief Pair wrapper creation helper. Similar to std::make_pair except there is no special
///        support for std::reference_wrapper (not needed for now).
template <typename Elem1Ty, typename Elem2Ty>
auto make_pair(Elem1Ty&& first, Elem2Ty&& second)
    -> pair<std::decay_t<Elem1Ty>, std::decay_t<Elem2Ty>>
{
    return {std::forward<Elem1Ty>(first), std::forward<Elem2Ty>(second)};
}
...
inline std::ostream& print_help_message(std::ostream& help_msg_out,
                                        const std::shared_ptr<const executable_info>& exec_info)
{
    // ----------------------------------------------------------------------------------------------------------------
    // Help message.
...
// --------------------------------------------------------------------------------------------------------------------
// ADL-enabled parser / validators / printers for some command-line options.
...
/// ADL-accessible validation/parsing route for print_type enum.
///
/// The last two parameters represents match type for validator (pointer to matched type) and
/// int type (to properly order overloads).
///
...
///
/// @exception boost::program_options::validation_error Parsing/validation failed on value of an option.
inline void validate(boost::any& out_var, const std::vector<std::string>& values, print_type*, int)
{
    ...
    const std::regex val_pattern(
        "^(?:[0-9]+|verbose|performance|extended[_\\-]?testing)$",
        std::regex_constants::ECMAScript | std::regex_constants::icase | std::regex_constants::optimize);

    // Underlying type of print_type.
    ...
    // in lexical_cast).
    using format_pt = std::common_type_t<format_ut, unsigned>;
    if (std::regex_match(value, val_pattern))
    {
        try
        {
...
/// ADL-accessible validation/parsing route for cmdline::pair type.
///
/// The last two parameters represents match type for validator (pointer to matched type) and
/// int type (to properly order overloads).
///
    ...
    -> std::enable_if_t<std::is_unsigned<Elem1Ty>::value && std::is_unsigned<Elem2Ty>::value>
{
    ...
    // Type used for conversion/lexical_cast for Elem1Ty (to avoid problems with char types
    // in lexical_cast).
    using format_val1_pt = std::common_type_t<Elem1Ty, unsigned long long>;
    using format_val2_pt = std::common_type_t<Elem2Ty, unsigned long long>;
    std::smatch matches;
    if (std::regex_match(value, matches, val_pattern))
    {
        try
        {
            const auto val1_str = matches[1].str();
            const auto val2_str = matches[2].str();

            auto val1 = boost::numeric_cast<Elem1Ty>(boost::lexical_cast<format_val1_pt>(val1_str));
            auto val2 = boost::numeric_cast<Elem2Ty>(boost::lexical_cast<format_val2_pt>(val2_str));

            out_var = boost::any(make_pair(val1, val2));
        }
        catch (...)
        {
            throw bpo::invalid_option_value(value);
    ...
    -> std::enable_if_t<std::is_unsigned<Elem1Ty>::value && std::is_unsigned<Elem2Ty>::value, std::ostream&>
{
    ...
    out << to_string(val.first) << "x" << to_string(val.second); // locale-independent conversion.
    return out;
}
