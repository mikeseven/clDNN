...
void example_fully_connected() {
    ...
    auto weights = memory_obsolete::create({ engine::cpu, memory_obsolete::format::xb_f32,{ weight_x, weight_y } });

    auto& input_memory = input.as<const memory_obsolete&>();
    auto& weights_memory = weights.as<const memory_obsolete&>();
