...
void example_softmax_forward() {
    ...
    const uint32_t output_y      = 7, output_x      = 7, output_z      = 3, output_b      = 3,  // size of whole output buffer
                   ...
                   in_off_y      = 0, in_off_x      = 0, in_off_z      = 0, in_off_b      = 0;
    ...
    auto input  = memory::create({engine::cpu, memory::format::xb_f32, {input_y, input_x, input_z, input_b}});
    auto output = memory::create({engine::cpu, memory::format::xb_f32, {output_y, output_x, output_z, output_b}});
