...
struct engine_configuration
{
    ...
    const bool dump_program;             ///< Dump the OpenCL programs to files
    ...
    /// @param dump_program Dump the OpenCL programs to files
    /// @param options OpenCL compiler options string.
    /// @param single_kernel If provided, runs specific layer.
    engine_configuration(bool profiling = false, bool decorate_kernel_names = false, bool dump_program = false, const std::string& options = std::string(), const std::string& single_kernel = std::string())
        :enable_profiling(profiling), meaningful_kernels_names(decorate_kernel_names), dump_program(dump_program), compiler_options(options), single_kernel_name(single_kernel) {}

    engine_configuration(const cldnn_engine_configuration& c_conf)
        :enable_profiling(c_conf.enable_profiling != 0), meaningful_kernels_names(c_conf.meaningful_kernels_names != 0), dump_program(c_conf.dump_program != 0), compiler_options(c_conf.compiler_options), single_kernel_name(c_conf.single_kernel_name){}

    /// @brief Implicit conversion to C API @ref ::cldnn_engine_configuration
    operator ::cldnn_engine_configuration() const
    {
        return{ enable_profiling, meaningful_kernels_names, dump_program, compiler_options.c_str(), single_kernel_name.c_str() };
    }
};
