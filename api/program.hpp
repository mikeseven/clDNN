...
public:
    program(engine const& eng, topology const& tpl, build_options const& options = build_options())
        :_impl(check_status<cldnn_program>("program creation failed", [&](status_t* status)
            {
                auto options_refs = options.get_refs();
                return cldnn_build_program(eng.get(), tpl.get(), options_refs.data(), options_refs.size(), status);
            }))
    {}
