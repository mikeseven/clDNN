...
namespace cldnn{
    CLDNN_API half_impl::operator float() const
    {
        auto value = check_status<float>("conversion to float failed", [&](status_t* status)
        {
            return cldnn_half_to_float(_data, status);
    ...
    {
        _data = check_status<uint16_t>("conversion to half failed", [&](status_t* status)
        {
            return cldnn_float_to_half(value, status);
