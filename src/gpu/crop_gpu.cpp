...
struct crop_gpu : primitive_impl
{
    const crop_inst& _outer;
    ...
    static kd_selector_t<kernel_data, crop_inst, kd_optional_selector_t, neural::gpu::engine_info_internal::architectures, neural::gpu::engine_info_internal::configurations> ks;

    crop_gpu(const crop_inst& outer) :
        _outer(outer),
    ...
    static kernel_data set_kernel_data(const crop_inst& outer)
    {
        ...
        kd.gws0 = output_mem.get_layout().size.batch[0];   // B
        kd.gws1 = output_mem.get_layout().size.feature[0]; // F
        kd.gws2 = output_mem.get_layout().size.spatial[0] * output_mem.get_layout().size.spatial[1]; // X, Y
    ...
    static gpu::jit_constants get_jit_constants(const crop_inst& outer, const kernel_data& data)
    {
        ...
        gpu::jit_constants mem_consts{
            ...
            gpu::make_jit_constant("OUTPUT",               outer.output_memory().get_layout().size),
    ...
    static primitive_impl* create(crop_inst &arg) { return new crop_gpu(arg); };
};

crop_gpu::kernel_data set_default(const crop_inst& arg)
{
    crop_gpu::kernel_data kd = crop_gpu::set_kernel_data(arg);
    kd.crop_bfyx_used = (arg.input_memory().get_layout().size.format == cldnn::format::bfyx) ? true : false;
    kd.kernel_name = kernel_name;

    return kd;
}

kd_selector_t<crop_gpu::kernel_data, crop_inst, kd_optional_selector_t, neural::gpu::engine_info_internal::architectures, neural::gpu::engine_info_internal::configurations> crop_gpu::ks = {
...
namespace {
    struct attach {
        attach() {
            ...
            implementation_map<crop_inst>::add(std::make_tuple(cldnn::engine_types::ocl, data_types::f32, format::yxfb), val_fw);
            implementation_map<crop_inst>::add(std::make_tuple(cldnn::engine_types::ocl, data_types::f16, format::yxfb), val_fw);
            implementation_map<crop_inst>::add(std::make_tuple(cldnn::engine_types::ocl, data_types::f32, format::bfyx), val_fw);
            implementation_map<crop_inst>::add(std::make_tuple(cldnn::engine_types::ocl, data_types::f16, format::bfyx), val_fw);
        }
        ~attach() {}
    };
