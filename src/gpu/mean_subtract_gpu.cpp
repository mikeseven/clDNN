...
KERNEL(Mean_subtract_GPU)(const __global neural_memory* input_mem, __global neural_memory* output_mem, const __global neural_memory* mean_mem)
{
    ...
    const int feature_id = (global_id / batch_num) % INPUT_FEATURE_NUM_0;
    const int x = ((global_id / batch_num) / INPUT_FEATURE_NUM_0) % INPUT_SIZE_X;
    const int y = ((global_id / batch_num) / INPUT_FEATURE_NUM_0) / INPUT_SIZE_X;
    output[global_id] = input[global_id] - mean[x + MEAN_SIZE_X * (y + MEAN_SIZE_Y * (feature_id + MEAN_FEATURE_NUM_0 * (batch_id % MEAN_BATCH_NUM)))];
}
)__krnl";
