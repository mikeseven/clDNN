...
struct batch_norm_gpu : primitive_impl
{
    const batch_norm_inst& _outer;
    ...
    static kd_selector_t<kernel_data, batch_norm_inst, data_types, format::type, bool, kd_optional_selector_t, neural::gpu::engine_info_internal::architectures, neural::gpu::engine_info_internal::configurations> ks;

    batch_norm_gpu(const batch_norm_inst& outer):
        _outer(outer),
        _engine_info(outer.get_network().get_engine()->get_context()->get_engine_info()),
        _kernel_data(ks.get_kernel(outer, outer.input_memory().get_layout().data_type, outer.input_memory().get_layout().size.format, outer.desc()->use_global_stats, _engine_info.architecture, _engine_info.configuration)),
    ...
    static kernel_data set_kernel_data(const batch_norm_inst& outer)
    {
        ...
        kd.gws2 = input_mem.get_layout().size.spatial[0] * input_mem.get_layout().size.spatial[1]; // X, Y
    ...
    static gpu::jit_constants get_jit_constants(const batch_norm_inst& outer, const kernel_data& data)
    {
    ...
    cldnn::refcounted_obj_ptr<cldnn::event_impl> execute(const std::vector<cldnn::refcounted_obj_ptr<cldnn::event_impl>>& events) override
    {
        ...
        const auto& variance_mem = outer.variance_memory();  // variance
    ...
    static primitive_impl *create(batch_norm_inst &arg) { return new batch_norm_gpu(arg); };
};

batch_norm_gpu::kernel_data set_default_use_global_stats(const batch_norm_inst& arg)
{
...
batch_norm_gpu::kernel_data set_default(const batch_norm_inst& arg)
{
    batch_norm_gpu::kernel_data kd = batch_norm_gpu::set_kernel_data(arg);
...
kd_selector_t<batch_norm_gpu::kernel_data, batch_norm_inst, data_types, format::type, bool, kd_optional_selector_t, neural::gpu::engine_info_internal::architectures, neural::gpu::engine_info_internal::configurations> batch_norm_gpu::ks = {
...
namespace {
    struct attach {
        attach() {
            ...
            implementation_map<batch_norm_inst>::add(std::make_tuple(cldnn::engine_types::ocl, data_types::f32, format::yxfb), val_fw);
            implementation_map<batch_norm_inst>::add(std::make_tuple(cldnn::engine_types::ocl, data_types::f16, format::yxfb), val_fw);
        }
        ~attach() {}
    };
    attach attach_impl;
