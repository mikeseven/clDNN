            ...
            if(!zero_y)
            {
                for (uint j = 0; j < FILTER_SIZE_X; j++)
                {
                    ...
                    for(uint h = 0; h < FILTER_INPUT_FEATURE_NUM / 8; h++)
                    {
                        ...
                        {
                            _in[a] = as_float(intel_sub_group_block_read((const __global uint*)input + (input_idx + a * input_buffer_feature_num * STRIDE_SIZE_X)));
                        }
                        ...
                        input_idx += 8 * input_buffer_batch_num;
                    }
...
#endif
                    {
                        ...
                        for(uint a = 0; a < X_PER_WORK_ITEM; a++)
                        {
                            _tmp[a] = mad(input[input_idx + a * input_buffer_feature_num * STRIDE_SIZE_X], _filter, _tmp[a]);
                        }
