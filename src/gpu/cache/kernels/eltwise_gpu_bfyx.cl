...
KERNEL (eltwise_gpu_bfyx)(const __global UNIT_TYPE* input, __global UNIT_TYPE* output, const __global UNIT_TYPE* input2)
{
    ...
    const uint b = global_id % batch_num;
    const uint f = (global_id / batch_num) % INPUT_FEATURE_NUM;
    ...
    const UNIT_TYPE in2 = input2[(b % INPUT2_BATCH_NUM) + INPUT2_BATCH_NUM * (f + INPUT2_FEATURE_NUM * (x + INPUT2_SIZE_X * y))];
    uint result;
#if MAX_MODE_USED
