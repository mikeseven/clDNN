    ...
    uint split_idx)
{
    ...
    uint batch_idx = fm / INPUT_BATCH_NUM;
    uint feature_idx = fm % INPUT_BATCH_NUM;
    ...
    uint in_split_offset = split_idx * (INPUT_SIZE_Y + 2 * INPUT_PADDING_SIZE_Y) * (INPUT_SIZE_X + 2 * INPUT_PADDING_SIZE_X) * FILTER_INPUT_FEATURE_NUM; //offset to input feature no 'split_idx'
