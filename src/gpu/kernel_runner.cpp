...
namespace cldnn { namespace gpu {
    ...
    kernel_runner::kernel_runner(engine_impl::ptr engine_ptr, bool weights_and_bias_exist) :
        engine(engine_ptr),
    ...
    {
        auto api_engine = *reinterpret_cast<cldnn::engine*>(&engine);
        ...
        if (input_buffers.empty())
        {
            for (const auto& input : base_params.inputs)
            {
                ...
                input_buffers.push_back(memory::allocate(api_engine, { from_data_type(input.GetDType()), format::bfyx, tensor(1, 1, num_of_input_elements, 1) }));
            }
        }
        ...
        if (output_buffers.empty())
        {
            ...
            output_buffers.push_back(memory::allocate(api_engine, { from_data_type(base_params.output.GetDType()), format::bfyx, tensor(1, 1, num_of_output_elements, 1) }));
        }
        ...
        if (weights_and_bias_exist)
        {
            ...
            {
                weight_buffers.push_back(memory::allocate(api_engine, { from_weights_type(weights_bias_params.weights.GetDType()), format::bfyx, tensor(1, 1, num_of_weight_elements, 1) }));
            }
            ...
            if (!weights_bias_params.bias.empty())
            {
                if (bias_buffers.empty())
                {
                    ...
                    bias_buffers.push_back(memory::allocate(api_engine, { from_data_type(weights_bias_params.bias[0].GetDType()), format::bfyx, tensor(1, 1, num_of_bias_elements, 1) }));
                }
