...
file::arguments::arguments(neural::engine::type aengine, std::string aname, memory_obsolete::format::type aformat, std::vector<uint32_t> &asize)
    : engine(aengine)
    , name(aname)
    , output{{memory_obsolete::create({aengine, aformat, asize, true})}} {}

file::arguments::arguments(neural::engine::type aengine, std::string aname, memory_obsolete::format::type aformat)
    : engine(aengine)
    , name(aname)
    , output{{memory_obsolete::create({aengine, aformat, std::vector<uint32_t>()})}} {}
...
primitive file::create(file::arguments arg) {
    try {
        ...
        if(arg.output[0]!=null_primitive) {
            auto output = arg.output[0].as<const memory_obsolete&>().argument;
        ...
        // crete result with output owning memory_obsolete, load data into it
        auto result = std::unique_ptr<file>(new file({arg.engine, arg.name, memory_obsolete::create({arg.engine, header.format, *size, true})}));
        auto &buffer = result->output_memory(0); 
        auto count = buffer.count()*memory_obsolete::traits(buffer.argument.format).type->size;
