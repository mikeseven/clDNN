...
struct convolution_backward_reference : is_an_implementation {
    ...
    static void implementation(const void *ptr) {
        auto this_conv = static_cast<const convolution_backward *>(ptr);
        auto input     = static_cast<float*>(this_conv->input_memory(0).pointer);
        auto output    = static_cast<float*>(this_conv->output_memory(0).pointer);
        auto window    = static_cast<float*>(this_conv->argument.weight.as<const memory&>().pointer);
        auto bias      = static_cast<float*>(this_conv->argument.bias.as<const memory&>().pointer);
        ...
        if(input_buffer_size.size()  != output_buffer_size.size()) throw std::runtime_error("Convolution_backward input/output number of dimension does not match.");
        if(stride.size()             != output_buffer_size.size()) throw std::runtime_error("Convolution_backward stride/output number of dimension does not match.");
        if(input_memory_arg.format   != memory::format::yxfb_f32)  throw std::runtime_error("Convolution_backward reference uses yxfb_f32 format.");             // only yxfb_f32 format is supported
        if(input_memory_arg.format   != output_memory_arg.format)  throw std::runtime_error("Convolution_backward input/output data format does not match.");    // only yxfb_f32 format is supported
        if(input_memory_arg.format   != window_memory_arg.format)  throw std::runtime_error("Convolution_backward input/weights data format does not match.");   // only yxfb_f32 format is supported
        if(window_buffer_size.size() != output_buffer_size.size()) throw std::runtime_error("Convolution_backward window_size/output number of dimension does not match.");
        if(bias_buffer_size.size()   != 1)                         throw std::runtime_error("Convolution_backward biases isn't 1D vector.");
        if(bias_buffer_size[0]       != output_size[2])            throw std::runtime_error("Convolution_backward biases/output feature maps number does not match."); // todo need type traits for index of 'z' dimension
                                                                                                                                                                       // than this implementation will be format independent
        // general formula: output size = (input size - window size) / step + 1
        for(size_t i = 0; i < input_offset.size(); ++i){
            if(output_size[i] < (static_cast<int32_t>(input_buffer_size[i]) - input_offset[i]) / (stride[i] + 1) )
                throw std::runtime_error("Output size of convolution_backward is to small.");

            if(output_buffer_size[i] < output_size[i] + output_offset[i])
                throw std::runtime_error("Convolution_backward output buffer size is to small.");

            //todo remove after padding implementation
            if(input_offset[i] < 0)
                throw std::runtime_error("Convolution_backward doesn't support padding yet. Input offset must be positive.");
        }
