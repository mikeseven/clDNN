...
layout layout_optimizer::get_expected_layout(layout const& current_layout, data_type type, std::shared_ptr<const convolution> prim, layout const& output_or_weights_layout)
{
    ...
    case data_type::input: //convolution input
        ...
        if (layout_optimizer::convolution_bfyx_opt(current_layout, output_or_weights_layout, prim)
            ...
            (current_layout.format == format::bfyx && output_or_weights_layout.size.spatial[0] == 1 && output_or_weights_layout.size.spatial[1] == 1 &&
                prim->stride.spatial[0] == 1 && prim->stride.spatial[1] == 1 &&
                prim->input_offset.spatial[0] == 0 && prim->input_offset.spatial[1] == 0 && ((current_layout.size.feature[0] % 8) == 0 && ((current_layout.size.spatial[0] * current_layout.size.spatial[1]) % 16) == 0 &&
