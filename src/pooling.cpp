    ...
    , padding(padd) 
{
    ...
    auto output_memory = in.output[0].as<const memory&>().argument;
    // compute size of output after pooling (downsampling)
    //auto spatial_x = static_cast<uint32_t>(ceil((static_cast<float>(output_memory.size.spatial[0] - siz.spatial[0]) / static_cast<float>(strd.spatial[0])))) + 1;
    //auto spatial_y = static_cast<uint32_t>(ceil((static_cast<float>(output_memory.size.spatial[1] - siz.spatial[1]) / static_cast<float>(strd.spatial[1])))) + 1;
