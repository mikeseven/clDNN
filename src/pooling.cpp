...
struct pooling_reference : is_an_implementation {
    ...
    static void implementation(const void *ptr) {
        ...
        if(input_memory_arg.format  != memory_obsolete::format::yxfb_f32)  throw std::runtime_error("Pooling reference uses yxfb_f32 format."); //todo, only this format?
        ...
        nd::calculate_idx_obsolete<uint32_t> calc_in_idx(input_buffer_size);
        nd::calculate_idx_obsolete<uint32_t> calc_out_idx(output_buffer_size);
...
pooling::arguments::arguments( neural::engine::type  eng,
                               ...
                               memory_obsolete::format::type  o_frmt,
    ...
    : engine(eng)
    , mode(mode)
    , output( {memory_obsolete::create({eng, o_frmt, out_siz, true})} )
...
static std::map<implementation_key, std::function<is_an_implementation *(pooling &)>> implementation_map = {
    {std::make_tuple(engine::reference, memory_obsolete::format::yxfb_f32, memory_obsolete::format::yxfb_f32), pooling_reference::create}
};
