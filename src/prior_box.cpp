...
void prior_box_node::calc_result()
{
    ...
    for (size_t i = 0; i < argument.min_sizes.size(); i++) {
        CLDNN_ERROR_LESS_OR_EQUAL_THAN(id(), "Min size value at index: " + i, argument.min_sizes[i], "less or equal than 0", 0, "Min size must be positive.");
    }
    if (argument.max_sizes.size() > 0) {
    ...
    if (argument.variance.size() > 1) {
        ...
        for (size_t i = 0; i < argument.variance.size(); i++) {
            CLDNN_ERROR_LESS_OR_EQUAL_THAN(id(), "Varaiance value at index: " + i, argument.variance[i], "value", 0, "Variance must be positive.");
        }
    }
