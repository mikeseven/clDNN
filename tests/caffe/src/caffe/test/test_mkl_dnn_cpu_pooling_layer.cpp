...
class MKL_DNN_CPU_PoolingLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  MKL_DNN_CPU_PoolingLayerTest()
      : blob_bottom_(new Blob<Dtype>()),
        blob_top_(new Blob<Dtype>()),
        blob_top_mask_(new Blob<size_t>()) {}
  virtual void SetUp() {
    Caffe::set_random_seed(1701);
    blob_bottom_->Reshape(48, 3, 6, 5);
    // fill the values
  ...
  virtual ~MKL_DNN_CPU_PoolingLayerTest() {
    delete blob_bottom_;
    delete blob_top_;
...
TYPED_TEST(MKL_DNN_CPU_PoolingLayerTest, TestSetupPadded) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  PoolingParameter* pooling_param = layer_param.mutable_pooling_param();
  pooling_param->set_kernel_size(3);
...
TYPED_TEST(MKL_DNN_CPU_PoolingLayerTest, TestSetupGlobalPooling) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  PoolingParameter* pooling_param = layer_param.mutable_pooling_param();
...
/*
TYPED_TEST(MKL_DNN_CPU_PoolingLayerTest, PrintBackward) {
  LayerParameter layer_param;
...
TYPED_TEST(MKL_DNN_CPU_PoolingLayerTest, TestForwardMax) {
  this->TestForwardSquare();
  this->TestForwardRectHigh();
  this->TestForwardRectWide();
}

#if 0  // TODO: TopMax not used in MKL-DNN currently
TYPED_TEST(MKL_DNN_CPU_PoolingLayerTest, TestForwardMaxTopMask) {
  typedef typename TypeParam::Dtype Dtype;
...
#if 0  // TODO: backward not ready
TYPED_TEST(MKL_DNN_CPU_PoolingLayerTest, TestGradientMax) {
  typedef typename TypeParam::Dtype Dtype;
...
#endif
TYPED_TEST(MKL_DNN_CPU_PoolingLayerTest, DISABLED_TestForwardMaxPadded) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  PoolingParameter* pooling_param = layer_param.mutable_pooling_param();
  pooling_param->set_kernel_size(3);
  ...
  pooling_param->set_pool(PoolingParameter_PoolMethod_MAX);
  this->blob_bottom_->Reshape(48, 1, 3, 3);
  // Input:
  //     [ 1 2 4 ]
  //     [ 2 3 2 ]
  //     [ 4 2 1 ]
...
#if 0
TYPED_TEST(MKL_DNN_CPU_PoolingLayerTest, TestGradientMaxTopMask) {
  typedef typename TypeParam::Dtype Dtype;
  for (int kernel_h = 3; kernel_h <= 4; kernel_h++) {
    for (int kernel_w = 3; kernel_w <= 4; kernel_w++) {
      LayerParameter layer_param;
...
TYPED_TEST(MKL_DNN_CPU_PoolingLayerTest, DISABLED_TestForwardAve) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  PoolingParameter* pooling_param = layer_param.mutable_pooling_param();
  pooling_param->set_kernel_size(3);
  ...
  pooling_param->set_pad(1);
  pooling_param->set_pool(PoolingParameter_PoolMethod_AVE);
  this->blob_bottom_->Reshape(24, 1, 3, 3);
  FillerParameter filler_param;
...
#if 0  // TODO: backward
TYPED_TEST(MKL_DNN_CPU_PoolingLayerTest, TestGradientAve) {
  typedef typename TypeParam::Dtype Dtype;
  for (int kernel_h = 3; kernel_h <= 4; kernel_h++) {
    for (int kernel_w = 3; kernel_w <= 4; kernel_w++) {
      LayerParameter layer_param;
...
TYPED_TEST(MKL_DNN_CPU_PoolingLayerTest, TestGradientAvePadded) {
  typedef typename TypeParam::Dtype Dtype;
  for (int kernel_h = 3; kernel_h <= 4; kernel_h++) {
    for (int kernel_w = 3; kernel_w <= 4; kernel_w++) {
      LayerParameter layer_param;
