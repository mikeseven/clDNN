...
const int gbatch = 24;
const int gchannels = 8;

namespace caffe {
static auto engine =  neural::engine::cpu;

template <typename TypeParam>
class MKL_DNN_CPU_LRNLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  MKL_DNN_CPU_LRNLayerTest()
      : epsilon_(Dtype(5e-5)),
        blob_bottom_(new Blob<Dtype>()),
        blob_top_(new Blob<Dtype>()) {}
  virtual void SetUp() {
    Caffe::set_random_seed(1701);
    blob_bottom_->Reshape(gbatch, gchannels, 3, 3);
    // fill the values
  ...
  virtual ~MKL_DNN_CPU_LRNLayerTest() { delete blob_bottom_; delete blob_top_; }
...
template <typename TypeParam>
void MKL_DNN_CPU_LRNLayerTest<TypeParam>::ReferenceLRNForward(
...
TYPED_TEST_CASE(MKL_DNN_CPU_LRNLayerTest, ::testing::Types<CPUDevice<float> >);


TYPED_TEST(MKL_DNN_CPU_LRNLayerTest, TestSetupAcrossChannels) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  MKL_DNNLRNLayer<Dtype> layer(layer_param, engine);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_->num(), gbatch);
  EXPECT_EQ(this->blob_top_->channels(), gchannels);
  EXPECT_EQ(this->blob_top_->height(), 3);
  EXPECT_EQ(this->blob_top_->width(), 3);
}
#if 1
TYPED_TEST(MKL_DNN_CPU_LRNLayerTest, TestForwardAcrossChannels) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  MKL_DNNLRNLayer<Dtype> layer(layer_param, engine);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
...
#endif
TYPED_TEST(MKL_DNN_CPU_LRNLayerTest, TestForwardAcrossChannelsLargeRegion) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
...
#if 0
TYPED_TEST(MKL_DNN_CPU_LRNLayerTest, TestGradientAcrossChannels) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  MKL_DNNLRNLayer<Dtype> layer(layer_param, engine);
...
TYPED_TEST(MKL_DNN_CPU_LRNLayerTest, TestGradientAcrossChannelsLargeRegion) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
...
#if 0
TYPED_TEST(MKL_DNN_CPU_LRNLayerTest, TestSetupWithinChannel) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
...
TYPED_TEST(MKL_DNN_CPU_LRNLayerTest, TestForwardWithinChannel) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.mutable_lrn_param()->set_norm_region(
      LRNParameter_NormRegion_WITHIN_CHANNEL);
...
TYPED_TEST(MKL_DNN_CPU_LRNLayerTest, TestGradientWithinChannel) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.mutable_lrn_param()->set_norm_region(
      LRNParameter_NormRegion_WITHIN_CHANNEL);
