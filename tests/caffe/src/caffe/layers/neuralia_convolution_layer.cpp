...
NeuraliaConvolutionLayer<Dtype>::NeuraliaConvolutionLayer(const LayerParameter& param)
      : ConvolutionLayer<Dtype>(param),
        fwd_bottom_data  (new NeuraliaData<Dtype>()),
        fwd_top_data     (new NeuraliaData<Dtype>()),
        fwd_filter_data  (new NeuraliaData<Dtype>()),
        fwd_bias_data    (new NeuraliaData<Dtype>()),
        bwd_top_diff     (new NeuraliaDiff<Dtype>()),
        bwd_bottom_diff  (new NeuraliaDiff<Dtype>()),
        bwd_filter_diff  (new NeuraliaDiff<Dtype>()),
        bwd_bias_diff    (new NeuraliaDiff<Dtype>()) {}

template <typename Dtype>
void NeuraliaConvolutionLayer<Dtype>::compute_output_shape() {
...
template <typename Dtype>
NeuraliaConvolutionLayer<Dtype>::~NeuraliaConvolutionLayer() {}

template <typename Dtype>
void NeuraliaConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
void NeuraliaMemoryDescriptor<Dtype, is_diff>::convert_from_prv(void* prv_ptr, void* cpu_ptr)
{
  CHECK(prv_ptr);
  CHECK(cpu_ptr);
...
template <typename Dtype, bool is_diff>
Dtype* NeuraliaMemoryDescriptor<Dtype, is_diff>::get_converted_prv(
  Blob<Dtype>* blob, bool set_prv_ptr, NeuraliaMemoryDescriptor<Dtype, is_diff>* converted_in_fwd) {
  if (this->to_internal != nullptr)
  {
    ...
    else
    {
      ...
      CHECK(prv_mem_descriptor->get_descr_type() == PrvMemDescr::PRV_DESCR_NEURALIA);

      shared_ptr<NeuraliaMemoryDescriptor<Dtype, is_diff> > current_descr =
        boost::static_pointer_cast<NeuraliaMemoryDescriptor<Dtype, is_diff> > (prv_mem_descriptor);
...
template <>
void NeuraliaConvolutionLayer<double>::Forward_cpu(const vector<Blob<double>*>& bottom,
    const vector<Blob<double>*>& top) { NOT_IMPLEMENTED; }

template <>
void NeuraliaConvolutionLayer<double>::Backward_cpu(const vector<Blob<double>*>& top,
        const vector<bool>& propagate_down,
    const vector<Blob<double>*>& bottom) { NOT_IMPLEMENTED; }


template <typename Dtype>
void NeuraliaConvolutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top)
{
  int n, g;
  int iw, ih, ic;
...
template <typename Dtype>
void NeuraliaConvolutionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(NeuraliaConvolutionLayer);
#endif

INSTANTIATE_CLASS(NeuraliaConvolutionLayer);
}  // namespace caffe
