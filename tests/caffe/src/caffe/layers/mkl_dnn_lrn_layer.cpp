...
void MKL_DNNLRNLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  fwd_bottom_data_->memory_usr = memory::create({engine_, fwd_bottom_data_->layout_usr, {num_, {width_, height_}, channels_ }});
  fwd_top_data_->memory_usr    = memory::create({engine_, fwd_top_data_   ->layout_usr, {num_, {width_, height_}, channels_ }});
  fwd_bottom_data_->memory_prv = memory::create({engine_, fwd_bottom_data_->layout_prv, {num_, {width_, height_}, channels_ }});
  fwd_top_data_->memory_prv    = memory::create({engine_, fwd_top_data_   ->layout_prv, {num_, {width_, height_}, channels_ }});

  bwd_bottom_diff_->memory_usr = memory::create({engine_, bwd_bottom_diff_->layout_usr, {num_, {width_, height_}, channels_ }});
  bwd_top_diff_->memory_usr    = memory::create({engine_, bwd_top_diff_   ->layout_usr, {num_, {width_, height_}, channels_ }});
  bwd_bottom_diff_->memory_prv = memory::create({engine_, bwd_bottom_diff_->layout_prv, {num_, {width_, height_}, channels_ }});
  bwd_top_diff_->memory_prv    = memory::create({engine_, bwd_top_diff_   ->layout_prv, {num_, {width_, height_}, channels_ }});
