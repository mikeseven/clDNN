...
void MKL_DNNInnerProductLayer<Dtype>::LayerSetUp(
  ...
  bottom_data_ = boost::make_shared<MKL_DNNData<Dtype> >(
    ...
    memory::describe({engine_, usr_layout_in_out_, {batch, {{inputs}}, 1}}),
    memory::describe({engine_, prv_layout_in_out_, {batch, {{inputs}}, 1}}));

  top_data_ = boost::make_shared<MKL_DNNData<Dtype> >(
    usr_layout_in_out_, prv_layout_in_out_,
    memory::describe({engine_, usr_layout_in_out_, {batch, {{outputs}}, 1}}),
    memory::describe({engine_, prv_layout_in_out_, {batch, {{outputs}}, 1}}));

  weights_data_ = boost::make_shared<MKL_DNNData<Dtype> >(
    usr_layout_weights_, prv_layout_weights_,
    memory::describe({engine_, usr_layout_weights_, {1, {{1}},
      {outputs, inputs}}}),
    memory::describe({engine_, prv_layout_weights_, {1, {{1}},
      {outputs, inputs}}}));
  ...
  weights_diff_ = boost::make_shared<MKL_DNNDiff<Dtype> >(
    usr_layout_weights_, prv_layout_weights_,
    memory::describe({engine_, usr_layout_weights_, {inputs, {{outputs}}, 1}}),
    memory::describe({engine_, prv_layout_weights_, {inputs, {{outputs}}, 1}}));
...
template <typename Dtype>
void MKL_DNNInnerProductLayer<Dtype>::Reshape(
  const vector<Blob<Dtype>*>& bottom,
  const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void MKL_DNNInnerProductLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  if (bottom[0]->get_prv_descriptor_data() != nullptr) {
    ...
    if (bottom_descr->layout_prv != prv_layout_in_out_ &&
      ...
      bottom_data_ = boost::make_shared<MKL_DNNData<Dtype> >(
          ...
          memory::describe({engine_, neural::memory::format::fyxb_f32,
          {n, {{w, h}}, c}}));
      ...
      bottom_data_xb_ =  memory::describe({engine_,
        neural::memory::format::xb_f32, {n, {{w*h*c}}, 1}});
  ...
  if (bias_term_) {
      // 1D, so conversion not necessary, but this is consistent..
      auto bias = bias_data_->get_converted_prv(this->blobs_[1].get(), true);

      execute({ (bottom_data_xb_ != nullptr) ? bottom_data_xb_(bottom_data) :
                  bottom_data_->memory_prv(bottom_data),
...
template <typename Dtype>
void MKL_DNNInnerProductLayer<Dtype>::Backward_cpu(
  const vector<Blob<Dtype>*>& top,
  const vector<bool>& propagate_down,
  const vector<Blob<Dtype>*>& bottom) {
  // TODO
