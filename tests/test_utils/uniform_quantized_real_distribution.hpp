...
/// @def CXX14_CONSTEXPR_FKW_
/// @brief Keyword used for functions compatible only with C++14 or above constexpr limits.
#if defined(_MSC_VER) && _MSC_VER < 1900
    #error Visual Studio older than 2015 is not supported.
#elif defined(_MSC_VER) && _MSC_VER < 2000
    #define CXX14_CONSTEXPR_FKW_
    #define CXX14_CONSTEXPR_FKW_UNDEF_MACRO_
#else
    #define CXX14_CONSTEXPR_FKW_ constexpr
    #define CXX14_CONSTEXPR_FKW_UNDEF_MACRO_
#endif
...
private:
    ...
    struct data_parts
    {
        ...
        CXX14_CONSTEXPR_FKW_ void part_hilo_ord(const std::size_t index, const part_type val) noexcept
        {
        ...
        /// @param index Index of the part (@c 0-based). Must be lower than data_repr_parts_count.
        /// @param val   New value for selected part.
        CXX14_CONSTEXPR_FKW_ void part_lohi_ord(const std::size_t index, const part_type val) noexcept
        {
        ...
        CXX14_CONSTEXPR_FKW_ data_parts& set_bits(std::size_t from) noexcept
        {
        ...
        CXX14_CONSTEXPR_FKW_ data_parts operator ~() const noexcept
        {
        ...
        CXX14_CONSTEXPR_FKW_ data_parts operator &(const data_parts& rhs) const noexcept
        {
            data_parts result(data_parts_zero);
            for (std::size_t pi = 0; pi < data_repr_parts_count; ++pi)
        ...
        /// @param rhs Right-hand side.
        /// @return    Current instance of data_parts.
        CXX14_CONSTEXPR_FKW_ data_parts& operator &=(const data_parts& rhs) noexcept
        {
            for (std::size_t pi = 0; pi < data_repr_parts_count; ++pi)
        ...
        friend CXX14_CONSTEXPR_FKW_ bool operator ==(const data_parts& lhs, const data_parts& rhs) noexcept
        {
            for (std::size_t pi = 0; pi < data_repr_parts_count; ++pi)
            {
...
public:
    ...
    /// @param from Index (@c 0-based) indicating first bit to set.
    // ReSharper disable once CppDoxygenUndocumentedParameter
    CXX14_CONSTEXPR_FKW_ data_type_mask(const std::size_t from, create_by_pos_mark) noexcept
        : _mask(data_parts_zero)
    {
    ...
    static CXX14_CONSTEXPR_FKW_ data_type_mask create_from_position(const std::size_t from)
    {
    ...
    CXX14_CONSTEXPR_FKW_ data_type_mask(const std::size_t from, const std::size_t len,
                                        // ReSharper disable once CppDoxygenUndocumentedParameter
    ...
    /// @param len  Number of bits to set.
    /// @return     Mask with proper bits set (rest of bits is cleared).
    static CXX14_CONSTEXPR_FKW_ data_type_mask create_from_position(const std::size_t from, const std::size_t len)
    {
    ...
    CXX14_CONSTEXPR_FKW_ data_type_mask operator ~() const noexcept
    {
    ...
    CXX14_CONSTEXPR_FKW_ data_type_mask operator &(const data_type_mask& rhs) const noexcept
    {
    ...
    CXX14_CONSTEXPR_FKW_ data_type_mask& operator &=(const data_type_mask& rhs) noexcept
    {
        _mask &= rhs._mask;
        return *this;
    }
    ...
    /// @param rhs Right side to compare.
    /// @return @c true if both sides are equal; otherwise, @c false.
    friend CXX14_CONSTEXPR_FKW_ bool operator ==(const data_type_mask& lhs, const data_type_mask& rhs) noexcept
    {
...
#ifdef CXX14_CONSTEXPR_FKW_UNDEF_MACRO_
    #undef CXX14_CONSTEXPR_FKW_
    #undef CXX14_CONSTEXPR_FKW_UNDEF_MACRO_
#endif
...
public:
    ...
    static constexpr unsigned significand_max_bits = std::max(std::numeric_limits<result_type>::digits - 1, 0);
