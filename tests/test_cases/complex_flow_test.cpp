...
TEST(complex_flow, trivial_convolution_relu) {
    ...
    auto conv = convolution::create({ engine::reference, memory::format::yxfb_f32,{ input, weights, biases },{ 1,{ 2, 2 }, 1 }, padding::zero });
...
TEST(complex_flow, trivial_convolution_relu_pool) {
    ...
    auto conv = convolution::create({ engine::reference, memory::format::yxfb_f32,{ input, weights, biases },{ 1,{ 2, 2 }, 1 }, padding::zero });
    auto act = relu::create({ engine::reference, memory::format::yxfb_f32,conv });
    auto pool = pooling::create(
    {
        engine::reference,
        pooling::mode::max,
        output,
