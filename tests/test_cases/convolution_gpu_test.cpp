...
void generic_convolution_test(cldnn::format test_input_fmt, cldnn::format test_filter_fmt,
	...
	for (int s = 0; s < split; ++s) {
		...
		biases.push_back(memory::allocate(engine, { cldnn::type_to_data_type<T>::value,{ format::bfyx,{ 1,1,1,filter_o } } }));
	}
	...
	topology topology(
		...
		convolution(
			...
			{ format::bfyx, 0, { 1,1,stride_y, stride_x } },
...
TEST(convolution_f32_fw_gpu, basic_convolution_no_bias) {
    ...
    topology topology(
        ...
        convolution("conv", "input", { "weights" }, { format::bfyx,{ 0,0,0,0 } }, { format::bfyx, 0,{ 0,0,2,1 } }));
...
TEST(convolution_f32_fw_gpu, basic_convolution) {
	...
	topology topology(
		...
		data("weights", weights),
		data("biases", biases),
		convolution( "conv", "input", { "weights" }, { "biases" }, { format::bfyx, { 0,0,0,0 } }, { format::bfyx, 0, { 0,0,2,1 } }));
...
TEST(convolution_f32_fw_gpu, basic_wsiz2x2_wstr2x2_in4x4x1x1_nopad_random) {
    ...
    topology topology(
        ...
        data("weights", weights),
        data("biases", biases),
        convolution("conv", "input", {"weights"}, {"biases"}, {format::bfyx, {0,0,0,0}}, {format::bfyx, {1,1,2,2}})
    );
...
TEST(convolution_f32_fw_gpu, basic_ofm_wsiz2x1x2x1_in1x2x1_nopad) {
    ...
    topology topology(
        ...
        data("weights", weights),
        data("biases", biases),
        convolution("conv", "input", { "weights" }, { "biases" }, { format::bfyx, { 0,0,0,0 } }, { format::bfyx,{ 1,1,5,5 } })
    );
