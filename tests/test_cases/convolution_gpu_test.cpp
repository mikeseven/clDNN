...
VF flatten_4d(format format, VVVVF &data) {
	size_t a = data.size();
...
TEST(convolution_f32_fw_gpu, basic_convolution) {
	...
	topology topology(
		...
		data("biases", biases),
		convolution( "conv", "input", { "weights" }, { "biases" }, { format::yx, { 0,0 } }, { format::yx, 0, { 2,1 } }));
...
TEST(convolution_gpu, bfyx_b8_f32)
{
    ...
    auto weights = memory::allocate(engine, { data_types::f32, tensor(format::oiyx, { 1, 2, 2, 2 }) });
