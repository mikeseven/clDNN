...
public:
	...
	static std::vector<cldnn::primitive*> generate_specific_test_params()
	{
		...
		for (auto norm_region : norm_regions)
		{
			...
			all_layer_params.push_back(new normalization("lrn", "input", 3, 1.f, 5e-05f, 0.75f, norm_region, { format::yx,{ 0, 0 } }, { format::yx,{ 13, 6 } }));
			all_layer_params.push_back(new normalization("lrn", "input", 5, 17.19f, 0.079f, 0.19f, norm_region));
			all_layer_params.push_back(new normalization("lrn", "input", 5, 17.19f, 0.079f, 0.19f, norm_region, { format::yx,{ 0, 0 } }, { format::yx,{ 5, 11 },{ 19, 0 } }));
		}
	...
	memory generate_reference_typed(const memory& input)
	{
		...
		//Output is bfyx
        data_types dt = input.get_layout().data_type;
		auto output = memory::allocate( engine, cldnn::layout(dt, input.get_layout().size.add(lrn->output_padding().lower_size()).add(lrn->output_padding().upper_size()).transform(cldnn::format::bfyx, 0)) );
