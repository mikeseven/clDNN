...
//  [ 2.0]
    ...
    auto input_prim  = memory_obsolete::create({engine::cpu, memory_obsolete::format::yxfb_f32, {3, 3, 1, 1}, true});
    auto output_prim = memory_obsolete::create({engine::cpu, memory_obsolete::format::yxfb_f32, {1, 1, 1, 1}, true});
    ...
    auto& output_memory = output_prim.as<const memory_obsolete&>();
...
//  [ 2.0,  1.5]
    ...
    auto input_prim  = memory_obsolete::create({engine::cpu, memory_obsolete::format::yxfb_f32, {3, 3, 1, 1}, true});
    auto output_prim = memory_obsolete::create({engine::cpu, memory_obsolete::format::yxfb_f32, {2, 2, 1, 1}, true});
...
//  [ 0.5,  0.5]
    ...
    auto input_prim  = memory_obsolete::create({engine::cpu, memory_obsolete::format::yxfb_f32, {4, 4, 1, 1}, true});
    auto output_prim = memory_obsolete::create({engine::cpu, memory_obsolete::format::yxfb_f32, {2, 2, 1, 1}, true});
...
//  [-0.5,  1.5]         [ 1.0,  0.0]
    ...
    auto input_prim  = memory_obsolete::create({engine::cpu, memory_obsolete::format::yxfb_f32, {3, 3, 2, 2}, true});
    auto output_prim = memory_obsolete::create({engine::cpu, memory_obsolete::format::yxfb_f32, {2, 2, 2, 2}, true});
    auto pool_prim = pooling::create({engine::reference, pooling::mode::max, output_prim, input_prim, {1, 1, 1, 1}, {2, 2, 1, 1}, padding::type::zero});
...
//  [ 1.0]
    ...
    auto input_prim  = memory_obsolete::create({engine::cpu, memory_obsolete::format::yxfb_f32, {2, 2, 1, 1}, true});
    auto output_prim = memory_obsolete::create({engine::cpu, memory_obsolete::format::yxfb_f32, {2, 2, 1, 1}, true});
...
//  [ 0.0,  1.0]             [ 1.0,  0.0]
    ...
    auto input_prim  = memory_obsolete::create({engine::cpu, memory_obsolete::format::yxfb_f32, {2, 2, 2, 2}, true});
    auto output_prim = memory_obsolete::create({engine::cpu, memory_obsolete::format::yxfb_f32, {2, 2, 2, 2}, true});
    auto pool_prim = pooling::create({engine::reference, pooling::mode::max, output_prim, input_prim, {-1, -1, 0, 0}, {2, 2, 1, 1}, {2, 2, 1, 1}, padding::type::zero});
...
//  Pool mode: max
    ...
    for(uint32_t config = 0; config < 6; ++config)
    {
        ...
        for(auto format : {memory_obsolete::format::yxfb_f32})
        {
            uint32_t total_dimensions = memory_obsolete::traits(format).dimension;
            for(uint32_t spatial_dimensions = 1; spatial_dimensions <= total_dimensions; ++spatial_dimensions)
            {
                ...
                auto input_prim  = memory_obsolete::create({engine::cpu, format, input_sizes, true});
                auto output_prim = memory_obsolete::create({engine::cpu, format, output_sizes, true});
                ...
                auto& input_memory = input_prim.as<const memory_obsolete&>();
                auto& output_memory = output_prim.as<const memory_obsolete&>();
                ...
                ndimensional::calculate_idx_obsolete<uint32_t> calc_in_idx(input_sizes);
