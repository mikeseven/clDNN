...
TEST(softmax_xb_f32_fw, intrinsics_avx2_batch1_ref_compare) {
    ...
    for(uint32_t output_element = 0; output_element < static_cast<uint32_t>(output_memory.count()); ++output_element)
...
TEST(softmax_xb_f32_fw, intrinsics_avx2_batch24_sum_to_one) {
    const uint32_t x = 1000, b = 24;
...
TEST(softmax_xb_f32_fw, intrinsics_avx2_batch24_ref_compare) {
    const uint32_t x = 100, b = 24;
